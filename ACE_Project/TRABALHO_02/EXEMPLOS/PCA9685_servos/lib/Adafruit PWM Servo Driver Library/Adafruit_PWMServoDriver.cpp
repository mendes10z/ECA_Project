 ...
 *  @return if the reset was acknowledged (indicating that the driver is present)
 */
uint8_t Adafruit_PWMServoDriver::begin(uint8_t prescale) {
  _i2c->begin();
  if (!reset()) return 0;
  delay(10);
  if (prescale) {
 ...
 *  @return if the reset was acknowledged
 */
uint8_t Adafruit_PWMServoDriver::reset() {
  return !write8(PCA9685_MODE1, MODE1_RESTART);
  //delay(10);
}
...
uint8_t Adafruit_PWMServoDriver::write8(uint8_t addr, uint8_t d) {
  _i2c->beginTransmission(_i2caddr);
  _i2c->write(addr);
  _i2c->write(d);
  return _i2c->endTransmission();
